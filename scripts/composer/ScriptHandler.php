...
class ScriptHandler {
  ...
  public static function createRequiredFiles(Event $event) {
    ...
    // Check if composer install/update is run without --no-dev flag.
    if($event->isDevMode()) {
      // Prepare the settings.local file for installation.
      if (!$fs->exists($root . '/sites/default/settings.local.php')) {
        $fs->copy($root . '/../scripts/local/settings.local.php', $root . '/sites/default/settings.local.php');
      ...
      // Prepare the development.services.yml file for installation.
      if (!$fs->exists($root . '/sites/development.services.yml')) {
        $fs->copy($root . '/../scripts/local/development.services.yml', $root . '/sites/development.services.yml');
        $fs->chmod($root . '/sites/development.services.yml', 0666);
        $event->getIO()->write("Create a sites/development.services.yml file with chmod 0666");
      }
    }
